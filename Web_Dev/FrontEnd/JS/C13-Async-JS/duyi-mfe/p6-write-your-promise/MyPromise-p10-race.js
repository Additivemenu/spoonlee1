...
class MyPromise {
  ...
  /**
   * !静态方法, 返回一个Promise, 该Promise的状态取决于proms的执行
   * (proms是一个迭代器, 包含多个Promise (也可以包含非Promise的值 -> 此时按照 Promise.resolve(非promise的值) 处理)); 全部promise都成功, 则all返回的Promise成功, 否则失败)
   * 最终返回的Promise的成功顺序, 和proms中的顺序一致 (和完成顺序无关)
   * 只要有一个Promise失败, 则all返回的Promise失败, 原因是第一个失败的promise的reason
   *
   * 两个关键点
   * 1. 如何保证all返回的Promise的完成顺序和proms中的顺序一致
   * 2. 如何判断全部Promise都成功了
   * @param {*} proms
   * @returns
   */
  static all(proms) {
    return new MyPromise((resolve, reject) => {
      try {
        const results = [];
        let count = 0; // promise的计数, 比fulfilledCount先增加到proms.length
        let fulfilledCount = 0; // 成功promise的计数

        for (const p of proms) {
          let i = count; // 记录当前promise的index
          count++;

          //! force to wrap the p with MyPromise.resolve(), 因为p可能是非Promise的值
          MyPromise.resolve(p).then((data) => {
            // !当前promise 成功后的逻辑, 异步通过micro-task queue执行
            fulfilledCount++;
            results[i] = data; // !保持results中的数据和proms中的顺序一致
            console.log("current fulfilled count", fulfilledCount); // run in micro-task queue
        ...
        // boundary conditions - when pass proms as []
        if (count === 0) {
          resolve(results);
        }
        ...
        // handle edge cases - when pass proms but not a iterable object
        reject(error);
  ...
  /**
   * 所有promise有结果之后, 该方法返回的Promise完成 (不可能失败), 并且按照顺序将所有结果汇总
   *
   * @param {*} proms
   * @returns
   */
  ...
  /**
   * 返回的Promise的状态和值, 取决于proms中第一个有结果的Promise
   * @param {*} proms
   * @returns
   */
  static race(proms) {
...
const pro = MyPromise.race([pro1, pro2, MyPromise.resolve("a3"), "a4", null]);

pro.then(
  (results) => {
    console.log("my promise race fulfilled:", results);
  },
  (reason) => {
    console.log("my promise race rejected:", reason);
  },
);
