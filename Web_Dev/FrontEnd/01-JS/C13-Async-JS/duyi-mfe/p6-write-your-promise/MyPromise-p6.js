...
const pro2 = pro.finally((data) => {
  ...
  throw 123; //!这个throw value会传递到finally返回的Promise里
  return 123; //!这个return value不会传递到finally返回的Promise里
});

setTimeout(() => {
  console.log(pro); // MyPromise { _state: 'fulfilled', _value: 1, _handlers: [] }
  console.log(pro2); // MyPromise { _state: 'rejected', _value: 123, _handlers: [] }
}, 2000);
