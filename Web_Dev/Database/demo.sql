...
-- create an example test table to show how to modify table.
...
-- end test example showing table modification.
...
-- selecting data where a column between 2 values
...
-- alias cannot be used in where claus
order by surname;
...
-- sum can only be used in int, numeric etc not string

select sum(movie_name) from movies; -- error occurrs as movie_name is string

select max(movie_length) from movies;

select max(release_date) from movies; -- latest date
select min(release_date) from movies; -- oldest date
...
-- Grouping data
select count(movie_lang) froom movies;
...
select movie_lang, age_certificate, avg(movie_length) from movies
group by movie_lang, age_certificate
where movie_length > 120; -- syntax error, where clause must be before group by
...
select movie_lang, count(movie_lang) from movies
where count(movie_lang) > 1     -- error occurred aggregate functions are not allowed in WHERE
group by movie_lang;
...
-- one to one relationships   e.g. movies movie_revenues
-- one to many relationships   e.g. directors movies
-- many to many relationships  e.g. movies  actors      junction table is: movies_actors

-- inner joins    will only return rows of data with matching values
-- left join      return all the rows of data in the left table and only matching rows of data in the right table
-- right join     return all the rows of data in the right table and only matching rows of data in the left table.
-- right join is opposite of the left join and mostly left join is used
-- full join:  return all the rows of data in both tables

-- inner join     after on  table primary key and foreign key should be used
select * from directors;
select * from movies;
...
-- left join -- the last record contains null
select d.director_id, d.first_name, d.last_name, mo.movie_name
from directors d
left join movies mo
...
-- right join  there is null record
select d.director_id, d.first_name, d.last_name, mo.movie_name
from directors d
...
left join movie_revenues mr on mr.movie_id = mo.movie_id;

-- find all the actor details who acts the movies that are English and order by movie_name
...
select column1, column2 from table2
 ...
 rules:
 1. should be same columns, and with compatible data types.
 */
 select first_name, last_name from directors
 union
 select first_name, last_name from actors;
 ...
 -- Intersect only find the name in both tables
 select first_name from directors
 intersect
 select first_name from actors;

 -- except     only return the name from table1 that doesn't exist in table2
 select first_name from directors
 except
 select first_name from actors;

 -- subqueries or nested queries  - can run subqueiry first and then get result and put the
 -- ourter queries, which is same as running the whole queries.
 select movie_name, movie_length from movies
 where movie_length >
 (select avg(movie_length) from movies);


 -- find all the directors who are younger than James Cameron
 ...
 -- find all the movies that international takings are larger than domestic takings
 ...
 -- find all the movies and related director and their movies international takings are larger than domestic takings
