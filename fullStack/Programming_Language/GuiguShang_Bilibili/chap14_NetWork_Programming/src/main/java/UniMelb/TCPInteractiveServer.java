...
public class TCPInteractiveServer {
	...
	public static void main(String[] args) {
		...
		try {
			//step1: Create a server socket listening on port 4444
			...
			while (true) 
			{
				...
				// step2: Accept an incoming client connection request
				...
				// step3: Get the input/output streams for reading/writing data from/to the socket
				BufferedReader in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
				BufferedWriter out = new BufferedWriter(new OutputStreamWriter(clientSocket.getOutputStream()));

				// step4: Read the message from the client and reply
